...
class mongodb_database(object):
    ...
    def insert_data(self, informartion, chat_id, multiple):
        ...
        if user is None and not multiple:
            # Create document
            user_data = {'chat_id': chat_id, 'phase': 1, 'reminders': 'daily'}
    ...
    def emotion_map(self, chat_id, value, column):
        if column == 'valence':
            ...
            if time_now >= '04:00' and time_now < '12:00':
                ...
                time_day = 'evening'
            information_dict = {'emotion_map.valence': value, 'emotion_map.time': time_day, 'emotion_map.date': calendar.day_name[datetime.today().weekday()]}
    ...
    def get_selected_conversation(self, chat_id, action):
        user = self.db.users.find_one({'chat_id': chat_id})
        if action == 'get':
            if 'last_conversation' in user.keys():
                la = user['last_conversation']
                if la < 3:
                    return la
                else:
                    new_la = np.random.randint(3)
                    self.db.users.update_one({'chat_id': chat_id}, {'$set': {'last_conversation': new_la}})
                    return new_la
            else:
                self.db.users.update_one({'chat_id': chat_id}, {'$set': {'last_conversation': 3}})
